...
def admin_get_couriers(login: str, password: str) -> Union[list, None]:
    if login_admin(login, password) != '-1':
        return request("http://host.docker.internal:5004/couriers", "App.get_couriers").data.result 
    return None
...
def admin_get_client(login: str, password: str, client_email: str, client_phone: str) -> Union[dict, None]:
    if login_admin(login, password) != '-1':
        return request("http://host.docker.internal:5005/client", "App.get_client", client_email=client_email, client_phone=client_phone).data.result
    else:
        return None
...
def admin_reg_client(login: str, password: str, client_email: str, client_phone: str, client_full_name: str) -> Union[str, None]:
    if login_admin(login, password) != '-1':
        return request("http://host.docker.internal:5005/client", "App.reg_client", email=client_email, password="", full_name=client_full_name, phone_num=client_phone).data.result
    else:
        return None
...
def admin_get_orders(login: str, password: str) -> Union[list, None]:
    if login_admin(login, password) != '-1':
        orders = request("http://host.docker.internal:5003/orders", "App.admin_get_orders").data.result
        couriers = request("http://host.docker.internal:5004/couriers", "App.get_couriers").data.result 
        for order in orders:
...
def post_order_admin(login: str, password: str, client_email: str, client_phone: str, 
                    sender_adr: str, sender_comment: str, sender_name: str, sender_coord_1: float, sender_coord_2: float, sender_phone: str,
    ...
    if login_admin(login, password) != '-1':
        client = request("http://host.docker.internal:5005/client", "App.get_client", client_email=client_email, client_phone=client_phone).data.result
        if "id" in client.keys():
            client_id = client["id"]
            return request("http://host.docker.internal:5003/orders", "App.post_order", 
...
if __name__ == '__main__':
    app.run(host="localhost", port=5000, debug=False) # port 5101
